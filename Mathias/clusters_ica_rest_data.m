...
stimulus_data_m = load(fullfile(volume_base2, path_to_data,"after_data_m.mat"));
...
total_nb_assemblies = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));
total_nb_neurons = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));
total_assemblies = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));
total_activity = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));
total_neurons_of_interest = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));
total_data = cell(size(stimulus_data_m,1),ceil(size(stimulus_data_m,2)/interval_size));

%% calculate assembly for each interval

% loop over each mouse
for k = 1:max(stimulus_data_m(:,1))
    cur_after_data = stimulus_data_m(stimulus_data_m(:,1) == k,:);
...
for i = 1:max(stimulus_data_m(:,1))
    % get last interval
    last_interval_index = ceil(size(stimulus_data_m,2)/interval_size);

    % create clustermatrix
    cluster_matrix = zeros(numel(find(stimulus_data_m(:,1)==i)), last_interval_index);
