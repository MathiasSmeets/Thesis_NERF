...
%% load data
path = "X:\Mathias\switch_data\frTables";
...
event_main_M = struct();
event_main_M.event_2 = load(fullfile(path, "frM2_stim.mat"), "-mat", "frM_stim");
...
for i = 1:numel(fields)
    ...
    cur_onsets_m = event_main_M.event_2.frM_stim.horridge{1,1};
    %cur_onsest_y = event_main_Y.event_2.frY_stim.horridge{1,1};
...
for i = 1:numel(fields)
    ...
    %cur_onsets_m = event_main_M.(fields{i}).events.onsets;
    %cur_onsets_y = event_main_Y.(fields{i}).events.onsets;
    cur_onsets_m = event_main_M.event_2.frM_stim.switch{1,1};
    %cur_onsest_y = event_main_Y.event_2.frY_stim.horridge{1,1};
    cur_onsets_m = cur_onsets_m(switcheventM(nb_mouse,1):end);
    cur_onsets_m(cur_onsets_m > cur_onsets_m(1)+10*60) = [];
    ...
    for j = 2:length(cur_onsets_m)
        if round(cur_onsets_m(j)*1000)+59 > 600000
            after_stimulus_switch_m{i,j-1} = data_m(start_neuron_m:start_neuron_m + cur_nb_neurons_m - 1,round(cur_onsets_m(j)*1000)-10+2:end);
            %after_stimulus_data_m{i,j-1}(start_neuron_m:start_neuron_m + cur_nb_neurons_m - 1, round(cur_onsets_m(j)*1000)+1:round(cur_onsets_m(j)*1000)+1) = 0;
        else
            after_stimulus_switch_m{i,j-1} = data_m(start_neuron_m:start_neuron_m + cur_nb_neurons_m - 1,round(cur_onsets_m(j)*1000)-10+2:round(cur_onsets_m(j)*1000)+59+2);
            %after_stimulus_data_m{i,j-1}(start_neuron_m:start_neuron_m + cur_nb_neurons_m - 1, round(cur_onsets_m(j)*1000)+1:round(cur_onsets_m(j)*1000)+1) = 0;
        end
    end
    % for j = 2:length(cur_onsets_y)
