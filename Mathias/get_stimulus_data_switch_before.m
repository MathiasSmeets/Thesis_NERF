...
for i = 1:numel(fields)
    ...
    for j = 2:length(cur_onsets_y)
        ...
        %     end
        % end
        if cur_onsets_y(j)*1000 < 600000
            if cur_onsets_y(j)-cur_onsets_y(j-1) > 0.12
                after_stimulus_switch_y{i,j-1} = switch_y(start_neuron_y:start_neuron_y + cur_nb_neurons_y - 1,round(cur_onsets_y(j)*1000)-59+2:round(cur_onsets_y(j)*1000)+10+2);
            end
        end
