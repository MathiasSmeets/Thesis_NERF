...
differences_y = (results_y(:,3)-results_y(:,1));%./results_m(:,1);

figure
scatter(switch_c(control_indices)-trainer_y(control_indices), differences_y(control_indices), 'filled')
hold on

% fit line to the data
p = polyfit(switch_c(control_indices)-trainer_y(control_indices), differences_y(control_indices), 1);
fit_line = polyval(p,switch_c(control_indices)-trainer_y(control_indices));

% r-value
ss_tot = sum((switch_c(control_indices)-trainer_y(control_indices) - mean(switch_c(control_indices)-trainer_y(control_indices))).^2);
ss_res = sum((switch_c(control_indices)-trainer_y(control_indices) - fit_line').^2);
r_squared = 1 - ss_res/ss_tot;

%plot(switch_c(learner_indices)-trainer_y(learner_indices), fit_line)
rho = corr((switch_c(control_indices)-trainer_y(control_indices))', differences_y(control_indices), 'Type', 'Spearman');
r = rho; % Correlation coefficient
n = 6; % Sample size
% Fisher transformation
z = 0.5 * log((1 + r) / (1 - r));
