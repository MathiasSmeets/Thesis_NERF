...
other_path = "X:\Mathias\switch_data\data_after_stimulus";
...
stimulus_data_m = load(other_path + "\before_stimulus_data_m_switch.mat");
...
stimulus_data_y = load(other_path + "\before_stimulus_data_y_switch.mat");
...
%save("/scratch/mathiass-takeokalab/01/neuron_spikes_y", "neuron_spikes_y","-v7.3");                                   %uncomment
    ...
    if ~isempty(stimulus_data_m{i,1})
        ...
        for j = 1:size(stimulus_data_m{i,1},1)
            ...
            for k = 1:size(peaks_locs_m{neuron_counter_m+j,2},2)
                if ~isempty(peaks_locs_m{neuron_counter_m+j,2})
                    ...
                    for ii = 1:size(stimulus_data_m,2)
                        if size(stimulus_data_m{i,ii},2) >= peak_interval_m(end)
            ...
            for jj = 1:size(peaks_to_keep_m,2)
                %[~, adjustment] = max(neuron_spikes_m(neuron_counter_m+j,peaks_to_keep_m(jj) - max_peak_distance:peaks_to_keep_m(jj)+ max_peak_distance));
                %cur_peak = peaks_to_keep_m(jj) + adjustment - max_peak_distance - 1;
                cur_peak = peaks_to_keep_m(jj);
                latencies = [];
                for kk = 1:size(stimulus_data_m,2)
                    if ~isempty(stimulus_data_m{i,kk})
                        if cur_peak + 5 > 70
                            peaks_this_interval = find(stimulus_data_m{i,kk}(j,cur_peak-5:end));
                            latencies = [latencies, peaks_this_interval];
                        elseif size(stimulus_data_m{i,kk},2) < cur_peak+5
                            peaks_this_interval = find(stimulus_data_m{i,kk}(j,cur_peak-5:end));
                            latencies = [latencies, peaks_this_interval];
                        else
                            peaks_this_interval = find(stimulus_data_m{i,kk}(j,cur_peak-5:cur_peak+5));
                            latencies = [latencies, peaks_this_interval];
                        end
                    end
                end

                if 1.6 > std(latencies) && 1.0 >= abs(cur_peak - mean(latencies + cur_peak - 5 - 1))
