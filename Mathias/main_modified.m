...
stimulus_data_high_fs_m = load("C:\Users\Mathi\OneDrive\Documenten\Master_3\Thesis\code\Mathias\data\data_after_stimulus_m_HIGHFS_NEW.mat");
stimulus_data_high_fs_y = load("C:\Users\Mathi\OneDrive\Documenten\Master_3\Thesis\code\Mathias\data\data_after_stimulus_y_HIGHFS_NEW.mat");
                ...
                for jj = 1:size(peaks_to_keep_m,2)
                    ...
                    if jj ~= 0
                        if cur_peak + max_peak_distance*10 > interval_size*10 % edge case
                            peak_area_m = neuron_spikes_m(j,cur_peak - 9 - max_peak_distance*10:end);                               % "j" should be "neuron_counter + j" in general case
                        elseif  cur_peak - max_peak_distance*10 < 1 % edge case
                            peak_area_m = neuron_spikes_m(j,1:cur_peak + max_peak_distance*10);                                     % "j" should be "neuron_counter + j" in general case
                        else % normal case
                            peak_area_m = neuron_spikes_m(j,cur_peak - 9 - max_peak_distance*10:cur_peak+max_peak_distance*10);     % "j" should be "neuron_counter + j" in general case
                        end
                    end
                    ...
                    for kk = 1:size(peak_area_m,2)
                        if cur_peak - max_peak_distance < 1
                            latency = kk-max_peak_distance-1 - (cur_peak - max_peak_distance-1);
                        else
