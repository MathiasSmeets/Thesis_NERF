                ...
                for jj = 1:size(peaks_to_keep_m,2)
                    ...
                    if jj ~= 0
                        if cur_peak + max_peak_distance*10 > interval_size*10 % edge case
                            peak_area_m = neuron_spikes_m_hfs(j,cur_peak - 9 - max_peak_distance*10:end);                               % "j" should be "neuron_counter + j" in general case
                        elseif  cur_peak - max_peak_distance*10 < 1 % edge case
                            peak_area_m = neuron_spikes_m_hfs(j,1:cur_peak + max_peak_distance*10);                                     % "j" should be "neuron_counter + j" in general case
                        else % normal case
                            peak_area_m = neuron_spikes_m_hfs(j,cur_peak - 9 - max_peak_distance*10:cur_peak+max_peak_distance*10);     % "j" should be "neuron_counter + j" in general case
                        end
                    end
