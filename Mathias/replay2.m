    ...
    for j = 1:last_interval_data(i)
        for k = 1:size(stimulus_data_m{i,j},2)-size(cur_template,2)+1
            if sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(cur_template,2)-1), 2) > 0) >= 2
                ...
                adj_cur_correlation_between(i,j) = adj_cur_correlation_between(i,j)/sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(cur_template,2)-1), 2) > 0);
            end
        end
    end
    ...
    for j = 1:last_interval_data(i)
        for k = 1:size(stimulus_data_m{i,j},2)-size(compressed_02,2)+1
            if sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_02,2)-1), 2) > 0) >= 2
                ...
                adj_cur_correlation_between_02(i,j) = adj_cur_correlation_between_02(i,j)/sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_02,2)-1), 2) > 0);
            end
        end
    end
    ...
    for j = 1:last_interval_data(i)
        for k = 1:size(stimulus_data_m{i,j},2)-size(compressed_05,2)+1
            if sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_05,2)-1), 2) > 0) >= 2
                ...
                adj_cur_correlation_between_05(i,j) = adj_cur_correlation_between_05(i,j)/sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_05,2)-1), 2) > 0);
            end
        end
    end
    ...
    for j = 1:last_interval_data(i)
        for k = 1:size(stimulus_data_m{i,j},2)-size(compressed_3,2)+1
            if sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_3,2)-1), 2) > 0) >= 2
                ...
                adj_cur_correlation_between_3(i,j) = adj_cur_correlation_between_3(i,j)/sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_3,2)-1), 2) > 0);
            end
        end
    end
    ...
    for j = 1:last_interval_data(i)
        for k = 1:size(stimulus_data_m{i,j},2)-size(compressed_5,2)+1
            if sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_5,2)-1), 2) > 0) >= 2
                ...
                adj_cur_correlation_between_5(i,j) = adj_cur_correlation_between_5(i,j)/sum(sum(stimulus_data_m{i,j}(cur_cluster,k:k+size(compressed_5,2)-1), 2) > 0);
            end
        end
    end
end
