...
ica_neurons_of_interest_before = load(fullfile(volume_base2, path_to_clusters, "neurons_of_interest_switch_m.mat"));ica_neurons_of_interest_before = ica_neurons_of_interest_before.total_neurons_of_interest;
ica_assemblies_before = load(fullfile(volume_base2, path_to_clusters, "assemblies_switch_m.mat")); ica_assemblies_before = ica_assemblies_before.total_assemblies;
%% get template
            ...
            if isequal(ica_neurons_of_interest{i,j}(ica_assemblies{i,j}{k}), most_common_cluster)
                if length(ica_activity{i,j}(:,k)) >= 3
                    ...
                    for jj = 1:length(locs)
                        ...
                        cur_raw_data = switch_data_m{i,raw_data_index};
