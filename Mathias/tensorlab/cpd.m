...
p.addOptional('NonnegativeModes', []);
...
% Parse options
options.UseEfficientRepresentation = ~isempty(options.NonnegativeModes) || ...
    ~isempty(options.VariablesInMode);
...
if options.UseEfficientRepresentation && exist('V', 'var')
    U = cellfun(@(v,u) v*u, V, U, 'UniformOutput', false);
end 
vidx = options.VariablesInMode;
if options.UseEfficientRepresentation && ~isempty(vidx) && max(vidx) == numel(U)+1
    U = [U, {ones(1,R)}];
end
% Select efficient representation in the case of constraints
if xsfunc(options.Compression) && options.UseEfficientRepresentation
    T = {V,T};
end 

if ~isempty(options.VariablesInMode) || ~isempty(options.NonnegativeModes)
    print('Step 3: Applying constraints to initialization...\r\n');
end 
...
if ~isempty(vidx)
    ...
    if strcmpi(output.Initialization.Name, 'cpd_rnd')
        % If cpd_rnd, set symmetric
        U = U(vidx);
    else 
        % else, compute best symmetric rank-1 approximation to each r-1 term
        for k = 1:max(vidx)
            ind = find(vidx == k);
            if numel(ind) <= 1, continue; end
            if mod(numel(ind), 2) == 0
                signs = (-2*(sum(prod(cat(3,U{ind}),3),1) < 0)+1);
                allsigns = allsigns .* signs;
            end
            % for r = 1:R
            %     z = cellfun(@(u) u(:,r), U(ind), 'UniformOutput', false);
            %     z = ccpd_nls({z}, {z{1},1}, {[ones(1,numel(ind)),2]}, 'MaxIter', 50);
            %     z{:}
            %     z = sign(z{2}) * z{1} * nthroot(abs(z{2}), numel(ind));
            %     z
            %     for i = ind, U{i}(:,r) = z; end 
            % end
        end
        % try to put signs in a mode that is not constrained by sign
        if any(allsigns < 0)
            counts = histc(vidx, 1:max(vidx));
            mode = find(counts > 0 & mod(counts,2) == 1, 1, 'last');
            if ~isempty(mode), 
                ind = find(vidx == mode);
                U{ind(1)} = bsxfun(@times, U{ind(1)}, allsigns);
            end 
        end 
    end 
    [~,idx,tmp] = unique(vidx);
    U = U(idx);
    err = frobcpdres(T, U(tmp))/frob(T);
    print('| Step 3a: Symmetry... relative error = %.6g\r\n', err);
end

%% Step 3b: nonnegativity
% Project negative values in nonnegative modes to zero, but try to affect the result as little as
% possible
if ~isempty(options.NonnegativeModes)
    vidx = options.VariablesInMode;
    if ~isempty(vidx)
        nonnegmodes = false(1, max(max(vidx), N));
    else 
        nonnegmodes = false(1, N);
    end 
    nonnegmodes(options.NonnegativeModes) = true;
    
    if isempty(vidx), vidx = 1:numel(U); end 
    [tmp,i,j]  = unique(vidx);
    powervars  = histc(vidx, tmp);
    powermodes = powervars(j);
    nonnegvars = nonnegmodes(i);
    
    allsigns = ones(1,R);
    for n = find(nonnegvars)
        signs    = (sum(sign(U{n}).*abs(U{n}).^2,1) < 0)*(-2) + 1;
        allsigns = allsigns .* (signs.^powervars(n));
        U{n}     = max(bsxfun(@times, U{n}, signs), 0);
    end 
    
    % try to put signs in a mode that is not constrained by sign
    mode = find(mod(powermodes,2) == 1 & ~nonnegmodes, 1, 'last');
    if ~isempty(mode) 
        ind = find(tmp == vidx(mode), 1, 'last');
        U{ind} = bsxfun(@times, U{ind}, allsigns);
    end 
    options.NonnegativeVariables = nonnegvars;

    [~,~,tmp] = unique(vidx);
    err = frobcpdres(T, U(tmp))/frob(T);
    print('| Step 3%c: Nonnegativity... relative error = %.6g\r\n', 'a'+~isempty(options.VariablesInMode), ...
          err);
end

%% STEP 3
step = 3 + options.UseEfficientRepresentation;
...
print(['Step %d: Algorithm is %s on the ' unstring 'compressed tensor'],step,func2str(options.Algorithm));
...
if xsfunc(options.Compression) && ischar(options.ExploitStructure) && strcmp(options.ExploitStructure,'auto')
    ...
    if ~options.ExploitStructure
        print('| Step %da: Detection of structure skipped (requested by user).\n', step);
    else
        supportedStructures = {'hankel'};
        supportedStructuresPrint = {'Hankel'};
        
        if strcmp(getstructure(T),'full')
            print('| Step %da: Detecting structure... ', step);
            ...
            else print('| Step %db: Converted tensor to efficient representation.\n',step); 
            end
        else
            print('| Step %da: Detection of structure skipped (%s structure already known).\n', ...
                  step, type);
        end
...
if xsfunc(options.Algorithm)
    if structureDetected
        % Structure detected
        print('| Step %dc: %s on the efficient representation... ',step,func2str(options.Algorithm));
    elseif xsfunc(options.Compression) && ischar(options.ExploitStructure) && ...
            strcmp(options.ExploitStructure,'auto')
        % No detection of structure
    else
        % No structure detected
        print('| Step %db: %s... ',step,func2str(options.Algorithm));
    end
    if ~isfield(options,'AlgorithmOptions')
        options.AlgorithmOptions = struct;
    end
    ...
    if ~isfield(options.AlgorithmOptions, 'LowerBound') && ~isempty(options.NonnegativeModes) 
        lb = -inf(1, numel(U));
        lb(options.NonnegativeVariables) = 0;
        options.AlgorithmOptions.LowerBound = num2cell(lb);
    end 
    if ~isfield(options.AlgorithmOptions, 'VariablesInMode') && ~isempty(options.VariablesInMode) 
        [~,~,vidx] = unique(options.VariablesInMode);
        options.AlgorithmOptions.VariablesInMode = vidx;
    end 
    ...
    warningstate = warning('off','cpd_core:accuracy');
    UA0 = U;
    [U,output.Algorithm] = options.Algorithm(T,UA0, options.AlgorithmOptions);
...
output.Algorithm.relerr = frobcpdres(T,U(vidx))/frob(T);
...
if structureDetected && output.Algorithm.info == 4
    print('| Step %dd: %s on the full tensor to improve accuracy... ',step,func2str(options.Algorithm));
    UB0 = U;
    ...
    output.AlgorithmFull.relerr = frobcpdres(Tfull,U(vidx))/frob(Tfull);
...
refinement_skipped_thresholdstructuredreached = isstructured && (frobcpdres(T,U)/frob(T) < continueRefinementThreshold);
...
if ~refinement_skipped_nodecompression && ~refinement_skipped_userrequested && ...
    ...
    print('Step %d: Refinement is %s on the uncompressed tensor:\n',step,func2str(options.Refinement));
    if options.ExploitStructure
        if ~strcmp(type,'full')
            print('| Step %da: Detection of structure skipped (%s structure already known).\n', ...
                  step, type);
...
elseif xsfunc(options.Refinement)
    ...
    if structureDetected
        print('| Step %dc: %s on efficient representation... ',step,func2str(options.Refinement));
    else
        print('| Step %db: %s... ',step,func2str(options.Refinement));
    end
    ...
    warningstate = warning('query', 'cpd_core:accuracy');
    ...
    output.Refinement.relerr = frobcpdres(T,U(vidx))/frob(T);
    print('relative error = %.6g.\n',output.Refinement.relerr);
    warning(warningstate.state,'cpd_core:accuracy');
...
if xsfunc(options.Refinement) && structureDetected && output.Refinement.info == 4
    print('| Step %dd: %s on the original full tensor to improve accuracy... ',step,func2str(options.Refinement));
    UB0 = U;
    ...
    output.RefinementFull.relerr = frobcpdres(T,U(vidx))/frob(T);
