...
%   - options.OutputFormat          - vector or factor matrices as output 
%        [{'vector'}, 'factors']
...
%
% Version History:
% - 2017/09/08   NV      Added output format option    
...
% parse options
    ...
    p.addOptional('OutputFormat', 'vector')
    p.parse(varargin{:});
    options = p.Results;
    ...
    if strcmpi(options.OutputFormat, 'factors')
        tmp = crb;
        crb = cell(size(U));
        offset = 0;
        for n = 1:numel(crb)
            crb{n} = reshape(tmp(offset + (1:numel(U{n}))), size(U{n}));
            offset = offset + numel(U{n});
        end 
    end 
