...
%
% Version History:
% - 2016/11/03   NV      Corrected random slices, output order and backup
...
if R > 1
    ...
    size_core = arrayfun(@(n)sum(sv{n} > tol(n)*max(sv{n})),1:getorder(T));
    ...
    if strcmpi(options.Slices, 'random')
        size_S = size(S);
        S = reshape(S, [size_S(1:2), prod(size_S(3:end))]);
    ...
    if any(R > size_core(1:2)) || size_core(3) ~= 2
        ...
        if ~options.IsReal && ~isfield(options, 'Imag') && isreal(T(:))
            if ~isfield(options, 'Real')
                options.Imag = @randn;
            else 
        ...
        if exist('iperm', 'var'), U = U(iperm); end
        return;
    end
