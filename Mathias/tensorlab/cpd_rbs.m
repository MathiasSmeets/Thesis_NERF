...
%   - 2014/06/12   NV      Initial version
    ...
    isfunctionhandle = isa(T, 'function_handle');
    if ~isfunctionhandle
        type = getstructure(T);
    ...
    elseif strncmp(func2str(options.Algorithm), 'minf', 4)
        ...
        dF = @grad;
        usestate = false;
    elseif strncmp(func2str(options.Algorithm), 'nls_', 4)
        % nls type algorithm
        dF.JHF = @grad;
    ...
    while ~output.info
        ...
        if isnumeric(T) || isfunctionhandle, 
            Ts = T(idx{:});
        else 
            Ts = ful(T, idx{:}); 
