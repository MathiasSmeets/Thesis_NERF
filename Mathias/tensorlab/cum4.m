function [c4,m4,c2] = cum4(X,prewhiten,blocksize,memlimit)
...
%
%   CUM4(X,[],blocksize) uses blocking with the specified blocksize to reduce
%   memory consumption. Default: 1e6.
%
%   CUM4(X,[],[],memlimit) sets the memory limit for the cumulant. An error
%   is thrown if size(X,1)^4 is larger than this limit. Default: 4*2^30 = 4GB.
%
...
%
%   Version history:
%   - 2016/04/11  NV  Added blocking and computation of mean without @mean
%   - 2014/12/16  NV  Improved speed and memory consumption

% Check the prewhiten option.
if nargin < 2 || isempty(prewhiten), prewhiten = false; end
if ischar(prewhiten), prewhiten = strcmpi(prewhiten,'prewhiten'); end
if nargin < 3 || isempty(blocksize), blocksize = min(1e6,size(X,1)); end
if nargin < 4 || isempty(memlimit), memlimit = 4*2^30; end
if size(X,2)^4*8 >= memlimit,
    error(['The requested cumulant would be %.1dGB > %.1dGB! Use memlimit to ' ...
           'change this bound.'], size(X,2)^4*8/2^30, memlimit/2^30);
end

% Center the variables.
m = ones(1,size(X,1))*X/size(X,1);
X = bsxfun(@minus,X,m);
...
for i = 1:blocksize:size(X,1)
    idx = i:min(i+blocksize-1,size(X,1));
    tmp = kr(X(idx,:)', X(idx,:).');
    m4 = m4 + tmp*tmp';
end    
