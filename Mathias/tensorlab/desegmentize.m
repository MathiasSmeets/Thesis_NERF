...
if ~isdefault.('Dims')
    ...
    if ~isdefault.('Dim')
        % dim is set
        error('desegmentize:dimanddims','Using both the ''Dim'' and ''Dims'' option arguments is invalid!');
    end
...
if ~strcmp(getstructure(S),'cpd') && ~isdefault.('Rank')
    error('desegmentize:rank','The rank option is not supported when S is not a CPD!');
end
