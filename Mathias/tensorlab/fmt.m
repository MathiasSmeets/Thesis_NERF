...
%
%   The following options can be set using an options structure or using
%   key value pairs:
% 
%   - TolSparsity   - If nnz(T)/numel(T) < TolSparsity, a dense tensor T is 
%                     converted to its sparse representation. Default: 0.05.

%   Authors: Laurent Sorber      (Laurent.Sorber@cs.kuleuven.be)
%            Nico Vervliet       (Nico Vervliet@cs.kuleuven.be)
%            Marc Van Barel      (Marc.VanBarel@cs.kuleuven.be)
%            Lieven De Lathauwer (Lieven.DeLathauwer@kuleuven-kulak.be)
%
% Version History:
% - 2016/02/28   NV      Automatic conversion to column vectors in structs
% - 2016/01/04   NV      Added handling of structured tensors

% Called internally?
if nargin >= 2 && islogical(varargin{1})
    varargin = [{'internal'}, varargin];
end
p = inputParser();
p.addOptional('Internal', false);
p.addOptional('TolSparsity', 0.05);
p.parse(varargin{:});
options = p.Results;
...
if isnumeric(T)
    ...
    elseif nnz(T)/numel(T) < options.TolSparsity
    ...
    try 
        ...
        if isstruct(T) && isfield(T, 'size') && isfield(T, 'val') && ...
                (isfield(T, 'ind') || isfield(T, 'sub'))
            type = 'incomplete';
        else 
            error('fmt:unknown', ...
                  'Could not determine the type of T (see getstructure(T))');
        end 
    end
    if ~any(strcmpi(type, {'full', 'sparse', 'incomplete'}))
        if isvalidtensor(T)
            ...
            error('fmt:invalidtensor', ['The given tensor is not a valid ' ...
                                'tensor (see isvalidtensor(T))']);
        end
    end
end
...
if isstruct(T)
    ...
    if ~isfield(T,'ind')
        error('fmt:ind','Data is missing field ''ind''.');
    end
    
    % Convert all to columns
    T.val = T.val(:);
    T.ind = T.ind(:);
    for n = 1:length(T.sub)
        T.sub{n} = T.sub{n}(:);
    end
