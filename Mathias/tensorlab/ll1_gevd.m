...
%      options.ClusterOnProjected  - Perform a projection step before
%      = [{true}|false]              clustering. This usually improves
%                                    performance, but may cause problems for
%                                    specific tensors.
...
% Version History:
% - 2017/04/08   NV      Option to disable projection step during clustering
...
% - 2014/11/24   NV      Initial version
    ...
    p.addOptional('ClusterOnProjected', true);
    ...
    if R == 1
        ...
        S = diag(S);
        U{1} = U{1}*diag(sqrt(abs(S)))*diag(sign(S));
        U{2} = U{2}*diag(sqrt(abs(S)));
        U = {[U, eye(sL)]};
        output.Backup = false;
    ...
    if sum(L) ~= length(L)
        ...
        Ctmp(isnan(Ctmp)) = 0;
        if options.ClusterOnProjected
            [u,~,~] = svd(Ctmp,'econ');
            tmp = abs(u(:,1:min(R, size(u,2)))'*Ctmp);
        else 
            tmp = Ctmp;
        end
