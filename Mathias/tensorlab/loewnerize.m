...
%               0.5.
%
%   - EquidistantLimit: If custom point set t is given, the point set is
%               considered equidistant if max(abs(diff(abs(diff(t)))) <
%               EquidistantLimit. By default: 2*eps.
%
...
%
%   Version History:
%   - 2016/10/27    NV      Added EquidistantLimit option.
...
p.addOptional('EquidistantLimit', 2*eps);
...
if nargout>1 || ~options.Full
    ...
    % equidistant
    if lstruct.order==2
        isequid = @(x) max(abs(diff(abs(diff(x))))) < ...
                  options.EquidistantLimit;
        if all(cellfun(@(x) length(x) <= 2 || isequid(t(x)),lstruct.ind))
