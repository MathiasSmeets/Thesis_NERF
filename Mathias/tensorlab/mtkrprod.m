...
if isstructured
    switch getstructure(T)
        ...
        case 'loewner'
            ...
            if n==T.dim
                ...
                DB = ifft(bsxfun(@times,fft(vf,[],1),fft(U{T.dim+1},numel(v),1)),[],1);
                DB = DB(size(U{T.dim+1},1):end,:);
                M1 = DB.*tmp;
                
                tmp = (g*kru).*U{T.dim+1};
                M2 = ifft(bsxfun(@times,fft(vf,[],1),fft(tmp,numel(v),1)),[],1);
    ...
    % If the last factor matrices have size(U{n},1) = 1, multiply them first
    % to avoid the creation of large intermediate results. As some methods
    % misuse mtkrprod, the additional test that all factor matrices should
    % have the same number of columns is required. 
    if ~left && any(size_tens(n+1:end) == 1) && all(cellfun('size',U,2)==size(U{1},2))
        n1 = max(find(size_tens > 1, 1, 'last'), max(n,2));
        if n1 < numel(U)
            U{n1} = bsxfun(@times, U{n1}, prod(vertcat(U{n1+1:end}),1));
            U(n1+1:end) = [];
            size_tens(n1+1:end) = [];
            N = n1;
        end
    ...
    if largescale
        ...
        idx = [1:n-1 n+1:N];
        if conjugate, U = cellfun(@conj,U,'UniformOutput',false); end
        if n == 0 || n > length(data.sub), M = zeros(1,R);
        else M = zeros(size_tens(n),R); end
        for r = 1:R
            s = data.val;
            for j = 1:length(idx)
                if idx(j) <= length(data.sub)
                    s = s.*U{idx(j)}(data.sub{idx(j)},r);
                else 
                    s = s.*U{idx(j)}(1,r);
                end 
            end
            if n == 0 || n > length(data.sub), M(r) = sum(s);
