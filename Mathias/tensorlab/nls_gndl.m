...
%
% Version History:
% - 2018/04/09    NV    Added custom printer
% - 2017/02/10    NV    Added measure and tolerance
...
dim = structure(z0);
z = z0;
if ~isempty(options.Preaction)
    z = options.Preaction(z0);
end 
z0 = serialize(z);    

% Set dependent defaults 
if isnan(options.Delta)
    options.Delta = 0.3*max(1,norm(z0));
end 
