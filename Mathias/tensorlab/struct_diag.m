function [x,state] = struct_diag(z,task,N)
...
%
%   struct_diag(z,taks,N) generates an Nth-order diagonal tensor of size
%   IxIx...xI with I = numel(z). 
%
...
if N == 2
    ...
    if isempty(task) || (isempty(task.l) && isempty(task.r))
        x = zeros(ones(1,N)*numel(z));
        x(linspace(1,numel(z)^N,numel(z))) = z(:);
    elseif ~isempty(task.r)
        x = zeros(ones(1,N)*numel(z));
        x(linspace(1,numel(z)^N,numel(z))) = task.r(:);
    elseif ~isempty(task.l)
        x = task.l(linspace(1,numel(z)^N,numel(z)));
        x = reshape(x, size(z));
    end
end
