...
%   If not provided, poly.type is by default Monomial and poly.domain is by
%   default [-1,1]
...
%
% Version History:
% - 2016/06/20    OD    Completion of input types/domains
...
function poly = completepoly(poly)

if ~isstruct(poly), error('poly should be a struct'); end
if ~any(strcmpi(fieldnames(poly),'domain'))
    poly.domain = [-1 1];
end
if ~any(strcmpi(fieldnames(poly),'type'));
    poly.type = 'monomial';
end
